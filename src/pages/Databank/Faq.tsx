...
const BestPractices: React.FC = () => {
  ...
  const formNames = [
    { header: t('form1HeaderText'), content: t('form1SubHeader') },
    { header: t('form2HeaderText'), content: t('form2SubHeader') },
    { header: t('form3HeaderText'), content: t('form3SubHeader') },
    { header: t('form4HeaderText'), content: t('form4SubHeader') },
    { header: t('form5HeaderText'), content: t('form5SubHeader') },
  ]
  ...
  return (
    <ThemeProvider theme={theme}>
      <div style={{ width: '100%', marginTop: '2rem' }}>
        ...
        <Grid
          sx={{ width: { xs: '90%', md: '60%', sm: '90%' } }}
          style={{ margin: 'auto', padding: '5px 5px 5px 5px', marginBottom: '15px' }}
        >
          {formNames.map((label, index) => (
            <div style={{ backgroundColor: '#DBE4FC' }}>
              <div style={{ display: 'flex', alignItems: 'center', padding: '10px 10px 0px 10px' }}>
                <a className='one' href={forms[index]} target='_blank'>
                  <IconButton aria-label='share'>
                    <SaveAltIcon />
                  </IconButton>
                </a>
                <Typography style={{ fontWeight: 'bold', marginLeft: '10px' }}>
                  {formNames[index].header}
                </Typography>
              </div>
              <Typography
                style={{ marginLeft: '60px', paddingBottom: '10px', marginBottom: '20px' }}
              >
                {formNames[index].content}
              </Typography>
            </div>
          ))}
        </Grid>
