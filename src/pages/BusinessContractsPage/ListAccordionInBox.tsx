...
import { Theme, ToggleButtonGroup, ToggleButton } from '@mui/material'
    ...
    switch (type) {
      ...
      default:
        return contracts
          ...
          .map((contract: any) => <ContractAccordion key={contract._id} contract={contract} />)
    }
  }

  const contractStatuses = [{ status: 'all' }, { status: 'pending' }, { status: 'signed' }]
        ...
        <div>
          ...
          >
            {contractStatuses.map((filter) => (
              <ToggleButton key={filter.status} value={filter.status}>
                {filter.status}
              </ToggleButton>
            ))}
          </ToggleButtonGroup>
