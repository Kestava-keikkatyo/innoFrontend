...
const ReportStepThree: React.FC<any> = ({ stepThreeError }) => {
  ...
  const handleSelectedType = (event: any) => {
    ...
    dispatch(setReport({ ...currentReport, type: valueForDB }))
  }
  ...
  const titleErrorProps =
    stepThreeError && title === ''
   ...
   * to submit report without title or details.*/
  const detailsErrorProps =
    stepThreeError && details === ''
  ...
  return (
    <Grid container style={{ marginTop: 16 }}>
      ...
      <Grid item xs={12}>
        <Typography variant='h2' className='header5'>
          {t('select_report_handler_report_title')}
        </Typography>
      </Grid>
      <Grid item xs={12}>
        <FormControl style={{ marginBottom: '1em' }}>
          ...
          >
            ...
            <MenuItem value='positive'>{t('select_report_handler_report_positive')}</MenuItem>
            <MenuItem value='deficiency'>{t('select_report_handler_report_deficiency')}</MenuItem>
            <MenuItem value='inappropriate'>
              {t('select_report_handler_report_inappropriate')}
            </MenuItem>
            <MenuItem value='closecall'>{t('select_report_handler_report_closecall')}</MenuItem>
            <MenuItem value='incident'>{t('select_report_handler_report_incident')}</MenuItem>
            <MenuItem value='else'>{t('select_report_handler_report_else')}</MenuItem>
          </Select>
        </FormControl>
      </Grid>
      {/**Title */}
