...
import { fetchEmploymentContractsAsWorkerOrBusiness } from '../../actions/contractActions'
...
const UserContractsPage = () => {
  ...
  if (employmentContracts.length) {
    employmentContracts.map((contract: any) => {
      if (
        (loadUser().role === 'worker' && !contract.workerSigned) ||
        (loadUser().role === 'business' && !contract.businessSigned)
      ) {
