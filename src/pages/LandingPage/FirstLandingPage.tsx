    ...
    <ThemeProvider theme={fontTheme}>
      <Container className="background">
        ...
        <div className="landing-bg-dark intro-container">
          <Typography className="text-center landing-text">
            {t('slogan')}
          </Typography>
          <Typography className="text-center landing-text2">
            {t('intro')}
          </Typography>

        </div>
        <Grid container spacing={4} sx={{ flexWrap: { xs: 'wrap', sm: 'wrap' } }} style={{ paddingTop: '32px' }} >
          <Grid item xs={12} sm={6} sx={{ backgroundClip: 'content-box', backgroundColor: '#FDFDFD', textAlign: { xs: 'center', sm: 'right' } }}>
            <Grid sx={{ boxShadow: 1, display: 'flex', flexDirection: 'column' }} style={{ textAlign: 'center', paddingTop: '20px', height: '100%' }}>
              <Container style={{ marginBottom: '10px', fontWeight: 'bold', fontSize: '24px' }}>{t('left_box_header')}</Container>
