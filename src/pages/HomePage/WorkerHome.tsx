...
import {
  Button,
  CardMedia,
  Grid, Link, ThemeProvider, Typography, createTheme
} from '@mui/material';
...
const WorkerHome = () => {
  ...
  return (
    <ThemeProvider theme={theme}>
      <Grid container>
        <Grid sx={{ height: { md: '40%', sm: '30%', xs: '20%' } }} style={{ marginBottom: '20px', maxHeight: '300px', width: '100%', display: 'flex', backgroundImage: 'linear-gradient(to bottom, #FDFDFD, #FDFDFD 50%, #C0CFFA 50%)', position: 'relative' }}>
        ...
        <Grid className={classes.generalInfo} sx={{ display: 'flex', flexDirection: { md: 'row', sm: 'row', xs: 'column' } }} >
          <Grid item className={classes.item} sx={{marginRight: {md: '20px', sm: '20px', xs: '0'}}}>
            <Typography align="center" variant="h6"> <Link style={{ color: 'black' }} href="/feeling/send" underline="hover">{t('feelings')}</Link> </Typography>
            <SendFeeling></SendFeeling>
          </Grid>
          <Grid item style={{margin: 0, paddingLeft: 12, paddingRight: 12}} className={classes.item}>
            <Typography align="center" variant="h6" style={{ marginBottom: '20px' }}> <Link style={{ color: 'black' }} href="/responsibilities" underline="hover">{t('responsibilities')}</Link> </Typography>
